...
const makeChatsSocket = (config) => {
    ...
    const resyncAppStateInternal = async (collections, fromScratch = false, returnSnapshot = false) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            for (const key in decoded) {
                ...
                if (snapshot) {
                    const newState = await Utils_1.decodeSyncdSnapshot(name, snapshot, getAppStateSyncKey);
                    states[name] = newState;
                    logger.info(`restored state of ${name} from snapshot to v${newState.version}`);
                }
                // only process if there are syncd patches
                if (patches.length) {
                    const { newMutations, state: newState } = await Utils_1.decodePatches(name, patches, states[name], getAppStateSyncKey, true);
    ...
    const appPatch = async (patchCreate) => {
        ...
        await mutationMutex.mutex(async () => {
            ...
            const { patch, state } = await Utils_1.encodeSyncdPatch(patchCreate, authState.creds.myAppStateKeyId, initial, getAppStateSyncKey);
            const node = {
                tag: 'iq',
                attrs: {
                    to: WABinary_1.S_WHATSAPP_NET,
